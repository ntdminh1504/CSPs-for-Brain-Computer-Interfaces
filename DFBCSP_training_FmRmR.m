function [ftr, fts, bands_selected_3] = DFBCSP_training_FmRmR(TRDATA,TRLB,TSDATA,params)
% Description:  Performs DFBCSP - Fisher’s ratio and mRmR on the input data.
%               
...
bands_selected = [bands(z(1,:)) bands(z(2,:))];
bands_selected_1 = [unique(bands_selected); unique(bands_selected+4)]';
% F_selected_1 = size(bands_selected_1,1);

%% (3) calculate mRmR of each subband
ftr=[];
for k=1:KTR
    f=[];
    for s=1:F
        W1{s} = train_csp(Xtr{s}, TRLB, params.m);
        X=Xtr{s}{k};
...
[idx, score] = fscmrmr(ftr, TRLB);
bands_selected = floor(idx(1:12)/6) + 1; % from feature index --> filterbank index
bands_selected = bands(bands_selected); % from filterbank index --> filterbank value
bands_selected_2 = [unique(bands_selected); unique(bands_selected+4)]';
% F_selected_2 = size(bands_selected_2,1);

%% (4) Find common FB 
bands_selected = intersect(bands_selected_1(:,1), bands_selected_2(:,1))';
if isempty(bands_selected)==1
    bands_selected_3 = bands_selected_2;
else
    bands_selected_3 = [unique(bands_selected); unique(bands_selected+4)]';
end
F_selected_3 = size(bands_selected_3,1);

%% filter bank of bands_selected
for s=1:F_selected_3
    for k=1:KTR
        Xtr_F{s}{k} = Xtr{(find(bands(:,1) == bands_selected_3(s,1)))}{k}; % channels x samples
    end
    for k=1:KTS
        x=TSDATA{k};
        Xts_F{s}{k} = eeg_filter(x', params.fs, bands_selected_3(s,1:2))'; % channels x samples
    end
