function [ftr, fts, bands_selected_2] = DFBCSP_training_mRmR(TRDATA,TRLB,TSDATA,params)
% Description:  Performs DFBCSP - mRmR on the input data.
%               
...
for s=1:F
    for k=1:KTR
       ...
       Xtr{s}{k} = eeg_filter(x', params.fs, bands(s,1:2))'; % channels x samples
    end
end

%% (2) calculate mRmR of each subband
ftr=[];
for k=1:KTR
    f=[];
    for s=1:F
...
bands_selected = floor(idx(1:6)/6) + 1; % from feature index --> filterbank index
...
for s=1:F_selected_2
    for k=1:KTR
        Xtr_F{s}{k} = Xtr{(find(bands(:,1) == bands_selected_2(s,1)))}{k}; % channels x samples
    end
    for k=1:KTS
        x=TSDATA{k};
        Xts_F{s}{k} = eeg_filter(x', params.fs, bands_selected_2(s,1:2))'; % channels x samples
    end
